...
Zasada
------
Ile warta jest jednostka roznicy sily, zalezy od planszy. Stosunek bierze sie
z tabel przepisanych w sasiednich modulach i wychodzi ten sam u kazdego, kto je
liczyl:

    plansza   19x19   13x13   9x9
    handi        13       5     2      (na 13 jednostek)

Podzielone przez 13 daje zmiane wyrownania na jednostke sily, a po przeliczeniu
na punkty przy RUCH = 13 — po prostu 13, 5 i 2 punkty na jednostke. Ze zrodlami
...
powtarzalnego ksztaltu. Drabinka kosztuje 5,2 punktu na jednostke zamiast 5,
...
co mowia zasady: Bialy dostaje szesciu jencow i wygrywa remisy. Potem komi
schodzi w dol az do -12 i dopiero
...
19x19 wychodzi z tego z komi -7 na kazdej jednostce od pierwszej wzwyz. Nie jest
to dziwactwo, tylko skutek tego, ze jednostka warta jest tam dokladnie jeden ruch:
...
za to ze zwyklym handicapem: jednostka roznicy to ruch wiecej.
...
# Ile punktow wyrownania dokłada jednostka roznicy sily — stosunek ze zrodel.
...
# Dlatego 13x13 dostaje jedna z tych koncowek na stale: co pol jednostki Czarny
...
#
# Cena: piec pozycji na 2,5 jednostki to 13 punktow, czyli 5,2 punktu na jednostke
# zamiast 5. Przy roznicy 20 daje to 2 punkty wiecej niz sam stosunek,
...
PIERWSZA_KRATKA_13X13 = 1.5

POLOWKA = 0.5

# Tak daleko, jak siega najdluzsza z przepisanych tabel (Ishikura) — zeby w
...
UWAGA = (
    ...
    + f" na jednostkę różnicy siły dla {'/'.join(KROK)}, przy ruchu wartym {RUCH} punktów."
...
def wiersz(plansza: str, roznica: float) -> tuple[float, int, int]:
    """(roznica, ruchy, komi) dla jednej roznicy sily — caly rachunek modulu.

    Roznica moze byc polowkowa, bo miedzy calymi liczbami tez sie gra. Polowka
    ...
    Dla roznic calkowitych trunc nie ma czego uciac (KROK jest calkowite), wiec
    tabele w TABELE, w JSON-ie i w zestawieniu wychodza co do liczby te same.
    """
    assert plansza in KROK, f"nieznana plansza: {plansza}"
    assert roznica * 2 == int(roznica * 2), f"roznica idzie co pol: {roznica}"
    if plansza == "13x13" and roznica >= PIERWSZA_KRATKA_13X13:
...
def _z_drabinki(roznica: float) -> tuple[int, int]:
    ...
    pozycja = round((roznica - PIERWSZA_KRATKA_13X13) / POLOWKA)
...
# 13x13 wypada z arytmetyki: na drabince pol jednostki to raz 3, raz 2 punkty,
